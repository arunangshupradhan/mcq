...
class Admin_model extends CI_Model
{
   ...
   public function get_mock_detail($id)
    {
        return $this->db->where('title_id', $id)
                ->get('exam_title')
                ->row();
    }
   ...
   // public function get_mock_detail($id)
   //  {
   //      return $this->db->select("exam_title.*, categories.category_id AS cats_id")
   //              ->where('title_id', $id)
   //              ->from('exam_title')
   //              ->join('categories', 'exam_title.category_id = categories.category_id')
   ...
   //  }
    ...
    public function add_question($file_name = '', $file_type = '')
    {
        ...
        if ($last_ques_id) {
            ...
            foreach ($_FILES['answer_image']['name'] as $key => $option) {
                if ($opt[$key]) {
                    $data['answer'] = $opt[$key];
                    ...
                    move_uploaded_file($_FILES['answer_image']['tmp_name'][$key], 'answer-images/' . $answer_id . '.jpg');
                }
            }
            return TRUE;
        } else {
    ...
    public function update_mock_title($id, $upload_data = '')
    {
        date_default_timezone_set($this->session->userdata['time_zone']);
    ...
    public function delete_category_name($id)
    {
        ...
        $have_exam = $this->db->get_where('exam_title', array('category_id' => $id), 1)->result();
        if ($have_exam) {
