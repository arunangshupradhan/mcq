...
class Message_control extends CI_Controller
{
    ...
    {
        $data = array();
        $data['class'] = 41; // class control value left digit for main manu rigt digit for submenu
    ...
    public function open_message($id = '', $message = '')
    {
        ...
        $data['sidebar'] = $this->load->view('sidebar/admin_sidebar', $data, TRUE);
        $data['message'] = $message;
        $data['msg'] = $this->admin_model->open_message($id);
        $data['msg_replies'] = $this->admin_model->get_replies($data['msg']->message_id);
