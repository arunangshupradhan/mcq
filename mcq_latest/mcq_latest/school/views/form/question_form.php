...
function add_form(val) {
  //  alert(val);
    ...
    var opt = '<div class=row><div class="col-xs-offset-1 col-xs-10 col-sm-offset-2 col-sm-8">';
        opt += '<p class="text-primary"><i class="glyphicon glyphicon-flash"></i> Select correct answer/s from left redio/checkbox options. You can add image as answer.</p>';
        opt += '</div></div>';
    ...
    for (q = 1; q <= 4; q++) {
        ...
        // }
        opt += '</div>';
        opt += '<input type="file" name="answer_image[' + q + ']" class="answer_image">';
        opt += '</div></div>';
    }
...
function add_field() {
    ...
    str += '</div>';
    str += '<input type="file" name="answer_image[' + i + ']" class="answer_image">';
    str += '</div></div>';
    str += '<div id="add_more_field-' + (i + 1) + '"></div>';
